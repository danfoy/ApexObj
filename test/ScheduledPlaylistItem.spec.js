...
describe('ScheduledPlaylistItem', function () {
  it('returns a superset of PlaylistItem', function () {
    ...
    const testPlaylistItem = new PlaylistItem(
      { mapName: 'we', mapDuration: 60, startTime: testStartTime },
      mockSeasonObj,
    );
    expect(
      new ScheduledPlaylistItem(
        { mapName: 'we', mapDuration: 60, startTime: new Date() },
        mockSeasonObj,
      ),
  ...
  it('throws if startTime is invalid', function () {
    expect(
      () =>
        new ScheduledPlaylistItem(
          { mapName: 'we', mapDuration: 60, startTime: 'zzz' },
          mockSeasonObj,
        ),
    ).to.throw();
  ...
  describe('.timeRemaining readonly property', function () {
    it('returns known correct values from season 11', function () {
      function check(date, mapDuration) {
        ...
        set(date);
        const timeRemaining = new RotatingPlaylist(
          seasonData.playlists[0],
          seasonData,
        ).currentMap.timeRemaining;
        expect(timeRemaining).to.equal(mapDuration * 1000 * 60);
        reset();

        // Time remaining half an hour into rotation
        const halfHourOffset = new Date(
          new Date(date).getTime() + 30 * 60 * 1000,
        );
        set(halfHourOffset);
        const offsetTimeRemaining = new RotatingPlaylist(
          seasonData.playlists[0],
          seasonData,
        ).currentMap.timeRemaining;
        expect(offsetTimeRemaining).to.equal(
          mapDuration * 1000 * 60 - 30 * 60 * 1000,
        );
        reset();
      }
    ...
    it('does not return negative values', function () {
      ...
      function check(date) {
        set(date);
        expect(
          new RotatingPlaylist(seasonData.playlists[0], seasonData).currentMap
            .timeRemaining,
        ).to.be.gt(0);
        reset();
      }
