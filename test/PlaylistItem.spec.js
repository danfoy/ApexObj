...
describe('PlaylistItem', function () {
  it('returns an object', function () {
    expect(new PlaylistItem(map('we', 30), playlist('br', false))).to.include({
      map: 'we',
      duration: 30,
      mode: 'br',
    });
  ...
  it('throws if map is not provided as a String', function () {
    ...
    expect(
      () => new PlaylistItem(map('ol', 60), playlist('br', false)),
    ).to.not.throw();
  });

  it('throws if playlist.mode is not provided', function () {
    expect(
      () => new PlaylistItem(map('we', 30), playlist('', false)),
    ).to.throw();
    expect(
      () => new PlaylistItem(map('we', 30), playlist(false, false)),
    ).to.throw();
    expect(
      () => new PlaylistItem(map('ol', 60), playlist('br', false)),
    ).to.not.throw();
  ...
  describe('.mode', function () {
    it('returns the mode of the parent playlist', function () {
      expect(new PlaylistItem(map('we', 30), season11BR).mode).to.equal(
        'Play Apex',
      );
    });
  });
