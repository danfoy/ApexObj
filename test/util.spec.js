...
import { isDate, parseDate, withinDates } from '../dist/util/date.js';
...
describe('Utility library', function () {
  ...
  describe('withinDates()', function () {
    ...
    it('returns false if before start of range', function () {
      const now = new Date().getTime();
      expect(
        withinDates({ startTime: now - 1000, endTime: now + 1000 }, now - 2000),
      ).to.be.false;
    });
    ...
    it('returns true if within range', function () {
      ...
      expect(
        withinDates({ startTime: now - 1000, endTime: now + 1000 }, now + 500),
      ).to.be.true;

      // With target not provided
      expect(withinDates({ startTime: now - 1000, endTime: now + 1000 })).to.be
        .true;
    });
  });
  ...
  describe('.randomFrom(list, quantity, options) function', function () {
    ...
    describe('Multiple item mode', function () {
      ...
      it('returns large arrays in non-subtractive mode', function () {
        expect(randomFrom([1, 2], 25, { subtractive: false })).to.be.lengthOf(
          25,
        );
      });
      it('only contains items from the input', function () {
        expect(
          randomFrom([1, 2], 25, { subtractive: false }),
        ).to.include.members([1, 2]);
        expect(randomFrom([1, 2], 25, { subtractive: false })).to.not.include(
          undefined,
        );
      });
      ...
      it("doesn't return duplicates in subtractive mode", function () {
        ...
        for (let i = 0; i < 1000; i++) {
          ...
          workingSet.forEach((entry) =>
            expect(workingSet.indexOf(entry)).to.equal(
              workingSet.lastIndexOf(entry),
            ),
          );
        }
      });
      ...
      it('throws when subtractive and requesting > input size', function () {
        expect(() => randomFrom(['item1', 'item2'], 2, { subtractive: true }))
          .to.throw;
      });
    });
  });
