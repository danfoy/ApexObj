...
describe('RotatingPlaylist', function () {
  ...
  const season12Control = season12.playlists.find(
    (playlist) => playlist.mode === 'Control',
  );
  ...
  describe('.rotations', function () {
    ...
    it('returns the correct number of rotations', function () {
      const totalRotations =
        season11BRData.maps.length * season11BRData.mapDurations.length;
  ...
  describe('.rotationBaseTime readonly property', function () {
    it('returns the baseTime if available', function () {
      expect(season11BR.rotationBaseTime).to.eql(
        new Date(season11BRData.baseTime),
      );
    });

    it('returns the startTime if baseTime not provided', function () {
      expect(season12Control.rotationBaseTime).to.eql(
        new Date(season12ControlData.startTime),
      );
    });
  });
  ...
  describe('.currentIndex readonly property', function () {
    function check(date, index) {
      set(date);
      expect(season11BR.currentIndex).to.equal(index);
      reset();
    }
    ...
    it("provides correct values for Season 11 'Escape'", function () {
      function check(date, mapName, duration, startOverride) {
        ...
        const testEndTime = new Date(
          testStartTime.getTime() + duration * 60 * 1000,
        );
  ...
  describe('.nextMap readonly property', function () {
    it('returns null if after season end', function () {
      ...
      const duringLastRotation = new Date(
        season11BR.endTime.getTime() - 1000 * 60 * 30,
      );
    ...
    it('returns the first rotation if before season start', function () {
      ...
      expect(season11BR.nextMap).to.eql(
        season11BR.getMapByDate(season11BR.startTime),
      );
      reset();
    });
    ...
    it("provides correct values for Season 11 'Escape'", function () {
      function check(date, mapName, duration) {
        set(date);
        expect(season11BR.nextMap).to.include({
          map: mapName,
          duration: duration * 60 * 1000,
        });
        expect(season11BR.nextMap.startTime).to.eql(
          season11BR.currentMap.endTime,
        );
        reset();
      }
  ...
  describe('#getPlaylistTimeElapsed(date)', function () {
    it('returns the time elapsed in this playlist rotation', function () {
      expect(
        season11BR.getPlaylistTimeElapsed('2022-01-24T02:00:00Z'),
      ).to.equal(120 * 60 * 1000);
    });
    it('passes regression tests', function () {
      expect(
        season11BR.getPlaylistTimeElapsed('2022-02-08T00:50:00Z'),
      ).to.equal(50 * 60 * 1000);
    });
  });
  ...
  describe('#getMapByDate(date)', function () {
    it('throws if the provided date is invalid', function () {
      ...
      expect(() =>
        season11BR.getMapByDate('2022-01-28T03:00:00Z'),
      ).to.not.throw();
    });

    it('uses the current date if none provided', function () {
      set('2022-01-11T12:00:00Z');
      expect(season11BR.getMapByDate()).to.include({
        map: "World's Edge",
        duration: 60 * 60 * 1000,
      });
      reset();
    ...
    it('passes regression tests', function () {
      ...
      expect(season11BR.getMapByDate().timeRemaining).to.equal(30 * 60 * 1000);
      reset();
    });
  });
});
