...
export function withinDates(dateRange: DateRange, target = new Date()) {
  if (!dateRange.startTime)
    throw new Error('startTime required, received ' + dateRange.startTime);
  if (!dateRange.endTime)
    throw new Error('endTime required, received' + dateRange.endTime);
